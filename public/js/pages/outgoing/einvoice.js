...
$(document).ready(function () {
        ...
        columnDefs: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return (
                        "<b>Fatura : </b>" +
                        moment(data.split("T")[0], "YYYY-MM-DD").format(
                            "DD.MM.YYYY"
                        ) +
                        "<br><b>Zarf : </b>" +
                        moment(row[10].split("T")[0], "YYYY-MM-DD").format(
                            "DD.MM.YYYY"
                        )
                    );
                },
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    return (
                        "<b>" +
                        data +
                        '</b><div class="progress-sm"><div class="progress-bar progress-bar-striped progress-sm bg-' +
                        type_class +
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    if (row[1]) {
                        ...
                        read_button =
                            '<div id="mark-readed"> <a class="dropdown-item" href="#">Okundu Olarak İşaretle</a></div>';
                    }
        ...
        ajax: {
            ...
            data: function (d) {
                return $.extend({}, d, {
                    ...
                    start_date: moment($("#fdate").val(), "DD-MM-YYYY").format(
                        "YYYY-MM-DD"
                    ),
                    end_date: moment($("#ldate").val(), "DD-MM-YYYY").format(
                        "YYYY-MM-DD"
                    ),
                    filter_array: document
                        .querySelector("#filter-select")
                        .getSelectedOptions(),
                    order_value: document.querySelector("#order-select").value,
                    order_direction:
                        document.querySelector("#direction-select").value,
                    read_marked: null,
                    profile_ids: null,
                    type_codes: null,
                });
    ...
    $("#invoices tbody").on("click", "#xml-download a", function () {
        ...
        let html;
        $.ajax({
            url: "/outgoing/einvoice/export/xml/" + data[0],
            type: "get",
            success: function (result) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.resultCode) {
                    ...
                    swal.fire("Hata!!", "Hata Detayı : " + response, "error");
                }
            },
        });
    });
    ...
    $("#invoices tbody").on("click", "#mark-unreaded a", function () {
        ...
        Swal.fire({
            title:
                data[3] +
                " numaralı faturayı okunmadı olarak işaretlemek istediğinize emin misiniz?",
            icon: "question",
    ...
    $("#invoices tbody").on("click", "#mark-readed a", function () {
        ...
        Swal.fire({
            title:
                data[3] +
                " numaralı faturayı okundu olarak işaretlemek istediğinize emin misiniz?",
            icon: "question",
            showCancelButton: true,
            confirmButtonText: `Evet`,
            cancelButtonText: `Hayır`,
    ...
    $("#invoices tbody").on("click", "#mark-unprinted a", function () {
        ...
        Swal.fire({
            title:
                data[3] +
                " numaralı faturayı yazdırılmadı olarak işaretlemek istediğinize emin misiniz?",
            icon: "question",
            showCancelButton: true,
            confirmButtonText: `Evet`,
            cancelButtonText: `Hayır`,
    ...
    $("#invoices tbody").on("click", "#mark-printed a", function () {
        ...
        Swal.fire({
            title:
                data[3] +
                " numaralı faturayı yazdırıldı olarak işaretlemek istediğinize emin misiniz?",
            icon: "question",
            showCancelButton: true,
            confirmButtonText: `Evet`,
            cancelButtonText: `Hayır`,
    ...
    $("#coll-pdf a").on("click", async function () {
        ...
        if (rows_selected.length == 0) {
            ...
            $.each(rows_selected, function (index, rowId) {
                $.ajax({
                    url: "/outgoing/einvoice/export/pdf/" + rowId,
                    type: "get",
                    async: false,
    ...
    $("#coll-xml a").on("click", async function () {
        ...
        if (rows_selected.length == 0) {
            ...
            $.each(rows_selected, function (index, rowId) {
                $.ajax({
                    url: "/outgoing/einvoice/export/xml/" + rowId,
                    type: "get",
                    async: false,
                    success: async function (result) {
                        const linkSource = `data:application/xml;base64,${result.content}`;
    ...
    $("#excel-lines a").on("click", async function () {
        ...
        if (rows_selected.length == 0) {
            ...
            $.ajax({
                ...
                url: "/outgoing/einvoice/uuids_lines_status",
                method: "post",
                data: JSON.stringify({
                    uuids,
                    direction: "out"
