...
$(document).ready(function () {
    ...
    let table = $('#despatches').DataTable({
        ...
        'columnDefs': [{
            ...
            },
            {
                ...
                "render": function (data, type, row) {
                    ...
                    return '<b>' + data.substring(0, 25) + threeDot + '</b><br> <b>VKN/TCKN : </b>' + row[12];
                },
                'width': '15%',
                'targets': 11
            },
        ...
        "serverSide": true,
        "ajax": {
            "url": "/outgoing/edespatch/getlist",
            "data": function (d) {
                return $.extend({}, d, {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status) {
                    ...
                    $.ajax({
                        url: '/outgoing/edespatch/export/html/' + data[0],
                        type: 'get',
    ...
    $('#despatches tbody').on('click', '#pdf-download a', function () {
        ...
        let html;
        $.ajax({
            url: '/outgoing/edespatch/export/pdf/' + data[0],
            type: 'get',
    ...
    $('#despatches tbody').on('click', '#xml-download a', function () {
        ...
        let html;
        $.ajax({
            url: '/outgoing/edespatch/export/xml/' + data[0],
            type: 'get',
            success: function (result) {
    ...
    $("#coll-pdf a").on("click", async function () {
        ...
        if (rows_selected.length == 0) {
            ...
            $.each(rows_selected, function (index, rowId) {
                $.ajax({
                    url: "/outgoing/edespatch/export/pdf/" + rowId,
                    type: "get",
                    async: false,
    ...
    $("#coll-xml a").on("click", async function () {
        ...
        if (rows_selected.length == 0) {
            ...
            $.each(rows_selected, function (index, rowId) {
                $.ajax({
                    url: "/outgoing/edespatch/export/xml/" + rowId,
                    type: "get",
                    async: false,
                    success: async function (result) {
                        const linkSource = `data:application/xml;base64,${result.content}`;
