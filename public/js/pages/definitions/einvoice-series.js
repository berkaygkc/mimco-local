$(document).ready(function () {
        ...
        Swal.fire({
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "GET",
                        url: `/definitions/series/1/default/${data.serie}`,
        ...
        Swal.fire({
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "GET",
                        url: `/definitions/series/1/activate/${data.serie}`,
                        success: function (response) {
                            if (response.status) {
                                Swal.fire({
                                    title: 'Başarılı!',
        ...
        Swal.fire({
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        type: "GET",
                        url: `/definitions/series/1/deactivate/${data.serie}`,
                        success: function (response) {
                            if (response.status) {
                                Swal.fire({
                                    title: 'Başarılı!',
    ...
    $('#new-einvoice-serie').click((e) => {
        Swal.fire({
            title: "Yeni e-Fatura serinizi giriniz!",
            input: "text",
            inputAttributes: {
            ...
            preConfirm: (serie) => {
                return fetch('/definitions/series/1/add/' + serie)
                    .then((response) => {
                        if (!response.status) {
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    title: "Başarılı",
                    text: "e-Fatura serisi başarı ile eklendi.",
                })
                .then(result => {
                    location.reload();
                })
